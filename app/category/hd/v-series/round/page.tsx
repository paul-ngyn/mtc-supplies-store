import AddToListButton from '@/components/ui/AddToListButton';
import Image from 'next/image';
import Link from 'next/link';

export default function VSeriesRoundPage() {
  // Product sizes/options - Real data from product catalog
  const sizeOptions = [
    { 
      id: 1, 
      itemNo: 'V-6016',
      size: '6" - 16 oz',
      dimensions: 'N/A',
      description: '6" 16 oz Round Black Vented Lid',
      caseQty: '150 SETS',
      caseWeight: 'N/A',
      palletCount: 'N/A',
      tiHi: 'N/A'
    },
    { 
      id: 2, 
      itemNo: 'V-7024',
      size: '7" - 24 oz',
      dimensions: 'N/A',
      description: '7" 24 oz Round Black Vented Lid',
      caseQty: '150 SETS',
      caseWeight: 'N/A',
      palletCount: 'N/A',
      tiHi: 'N/A'
    },
    { 
      id: 3, 
      itemNo: 'V-7032',
      size: '7" - 32 oz',
      dimensions: 'N/A',
      description: '7" 32 oz Round Black Vented Lid',
      caseQty: '150 SETS',
      caseWeight: 'N/A',
      palletCount: 'N/A',
      tiHi: 'N/A'
    },
    { 
      id: 4, 
      itemNo: 'V-7040',
      size: '7" - 40 oz',
      dimensions: 'N/A',
      description: '7" 40 oz Round Black Vented Lid',
      caseQty: '150 SETS',
      caseWeight: 'N/A',
      palletCount: 'N/A',
      tiHi: 'N/A'
    },
    { 
      id: 5, 
      itemNo: 'V-8037',
      size: '8" - 37 oz',
      dimensions: 'N/A',
      description: '8" 37 oz Round Black Vented Lid',
      caseQty: '150 SETS',
      caseWeight: 'N/A',
      palletCount: 'N/A',
      tiHi: 'N/A'
    },
    { 
      id: 6, 
      itemNo: 'V-9048',
      size: '9" - 48 oz',
      dimensions: 'N/A',
      description: '9" 48 oz Round Black Vented Lid',
      caseQty: '150 SETS',
      caseWeight: 'N/A',
      palletCount: 'N/A',
      tiHi: 'N/A'
    },
    { 
      id: 7, 
      itemNo: 'V-90348',
      size: '9" - 48 oz 3-Comp',
      dimensions: 'N/A',
      description: '9" 48 oz 3-Compartment Round Black Vented Lid',
      caseQty: '150 SETS',
      caseWeight: 'N/A',
      palletCount: 'N/A',
      tiHi: 'N/A'
    },
  ];

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-6 py-8">
        {/* Breadcrumb Navigation */}
        <nav className="flex items-center space-x-2 text-sm text-gray-500 mb-6">
          <Link href="/" className="hover:text-gray-700 transition-colors">
            Home
          </Link>
          <span>/</span>
          <Link href="/category/hd" className="hover:text-gray-700 transition-colors">
            HD
          </Link>
          <span>/</span>
          <Link href="/category/hd/v-series" className="hover:text-gray-700 transition-colors">
            V-Series
          </Link>
          <span>/</span>
          <span className="text-gray-700 font-medium">Round Vented Containers</span>
        </nav>

        {/* Product Section */}
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
            {/* Product Image */}
            <div className="relative h-96 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg overflow-hidden">
              <Image
                src="/TKN.png"
                alt="V-Series Round Vented Containers"
                fill
                className="object-contain p-8"
                priority
              />
            </div>

            {/* Product Info */}
            <div className="flex flex-col">
              <h1 className="text-4xl font-bold text-gray-900 mb-4">
                V-Series Round Vented Containers
              </h1>
              
              <div className="mb-6">
                <p className="text-gray-600 text-lg leading-relaxed">
                  Premium V-Series round containers featuring black vented lids for optimal freshness and temperature 
                  control. These high-quality containers are designed for hot food service, allowing steam to escape 
                  while maintaining food quality. Perfect for restaurants, takeout operations, and food service 
                  establishments. Available in sizes from 16 oz to 48 oz.
                </p>
              </div>

              <div className="mb-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-3">Features:</h2>
                <ul className="list-disc list-inside space-y-2 text-gray-600">
                  <li>Black vented lids for steam release</li>
                  <li>Ideal for hot food service</li>
                  <li>Professional presentation</li>
                  <li>Food-safe plastic materials</li>
                  <li>Wide capacity range (16-48 oz)</li>
                  <li>Includes 3-compartment option</li>
                </ul>
              </div>
              {/* Add to List Button */}
              <div className="mt-auto pt-6 border-t border-gray-200">
                <AddToListButton
                  productName="V-Series Round Vented Containers"
                  brand="HD"
                  category="V-Series Round Vented Containers"
                  slug="v-series/round"
                  brandSlug="hd"
                  image="/TKN.png"
                  sizes={sizeOptions.map(opt => `${opt.size} (${opt.itemNo})`)}
                  showSizeSelector={true}
                />
              </div>

            </div>
          </div>

          {/* Details / Options Section */}
          <div className="border-t border-gray-200 p-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Available Sizes & Specifications</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {sizeOptions.map((option) => (
                <div
                  key={option.id}
                  className="border border-gray-400 rounded-lg p-3 bg-white"
                >
                  <div className="mb-2 pb-2 border-b border-gray-100">
                    <h3 className="text-md font-bold text-gray-900 mb-0.5">
                      {option.size}
                    </h3>
                    <p className="text-sm font-semibold text-blue-600">
                      {option.itemNo}
                    </p>
                  </div>
                  
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">Case Qty:</span>
                      <span className="font-medium text-gray-900">{option.caseQty}</span>
                    </div>
                    {option.dimensions !== 'N/A' && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600">Dimensions:</span>
                        <span className="font-medium text-gray-900 text-right">{option.dimensions}</span>
                      </div>
                    )}
                    {option.caseWeight !== 'N/A' && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600">Case Wt:</span>
                        <span className="font-medium text-gray-900">{option.caseWeight}</span>
                      </div>
                    )}
                    {option.palletCount !== 'N/A' && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600">Pallet:</span>
                        <span className="font-medium text-gray-900">{option.palletCount}</span>
                      </div>
                    )}
                    {option.tiHi !== 'N/A' && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600">TI/HI:</span>
                        <span className="font-medium text-gray-900">{option.tiHi}</span>
                      </div>
                    )}
                  </div>
                </div>
              ))}
            </div>

            <div className="mt-8 p-6 bg-blue-50 rounded-lg">
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Want to place an order or have any product questions?</h3>
              <p className="text-gray-600 mb-4">
                Contact us for bulk orders or custom sizing options. We're here to help with your specific needs.
              </p>
              <Link 
                href="/contact" 
                className="inline-block bg-[#1c51a3] text-white px-6 py-3 rounded-md hover:bg-[#153d7f] transition-colors font-medium"
              >
                Contact Us
              </Link>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

// Generate metadata for SEO
export async function generateMetadata() {
  return {
    title: 'V-Series Round Vented Containers - HD Products - Maple Trade Corp',
    description: 'Premium V-Series round containers with black vented lids from 16oz to 48oz. Perfect for hot food service and takeout.',
  };
}
